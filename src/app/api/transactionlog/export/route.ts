...
/**
 * @swagger
 * /api/transactionlog/export:
 *   post:
 *     tags:
 *       - Transaction Logs
 *     summary: Track transaction log CSV export
  ...
  try {
    ...
    const importFileRecord = await prisma.importfile.create({
      data: {
        ...
        status: 'EXPORTED',
        errorCount: 0,
        remarks: remarks || `Transaction logs export - ${recordCount} records exported`,
        filePath: null
      }
    });
    ...
    console.error(' Transaction Log Export Tracking error:', error);
    return NextResponse.json(
      { 
        status: 'error',
        code: 500,
