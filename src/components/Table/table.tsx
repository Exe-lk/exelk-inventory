...
const Table: React.FC<TableProps> = ({
  ...
  const getActionButtonStyles = (variant: string = 'primary') => {
    const base = 'px-3 py-1.5 text-xs font-medium rounded-md transition-colors duration-200 focus:outline-none focus:ring-2 focus:ring-offset-1 dark:focus:ring-offset-slate-800';
                        ...
                        <svg 
                          className={`w-3 h-3 ${
                            sortColumn === column.key && sortDirection === 'asc' 
                              ? 'text-gray-900 dark:text-gray-200' : 'text-gray-400 dark:text-gray-500'
                          }`} 
                          fill="currentColor" 
                          viewBox="0 0 20 20"
                        >
          ...
          <tbody className="bg-white dark:bg-slate-800">
            {loading ? (
              <tr>
                <td colSpan={columns.length + (actions.length > 0 ? 1 : 0)} className="px-6 py-12 text-center">
                  <div className="flex justify-center items-center">
                    <div className="animate-spin rounded-full h-8 w-8 border-b-2 border-blue-500 dark:border-indigo-500"></div>
              ...
              paginatedData.map((row, index) => (
                <tr key={index} className={`border-b border-gray-100 dark:border-slate-700 hover:bg-gray-50 dark:hover:bg-slate-700/50 ${getRowClassName ? getRowClassName(row) : ''}`}>
                  {columns.map((column) => (
