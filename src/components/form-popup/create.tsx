...
const validateField = (field: FormField, value: any): string | null => {
  ...
  const renderField = (field: FormField) => {
    ...
    } ${field.disabled ? 'bg-gray-100 dark:bg-slate-700 cursor-not-allowed' : 'bg-white dark:bg-slate-700'} text-gray-900 dark:text-white placeholder-gray-400 dark:placeholder-gray-500`;
          ...
          <div className="relative">
            <select
              ...
              className={`${baseInputClasses} appearance-none pr-10 ${
                !formData[field.name] ? 'text-gray-400 dark:text-gray-500' : 'text-gray-900 dark:text-white'
              }`}
              disabled={field.disabled}
            >
              <option value="" disabled className="bg-white dark:bg-slate-700 text-gray-900 dark:text-white">
                {field.placeholder || `Select ${field.label}`}
              </option>
              {field.options?.map((option) => (
                <option key={option.value} value={option.value} className="bg-white dark:bg-slate-700 text-gray-900 dark:text-white">
                  {option.label}
                </option>
              ))}
            </select>
  ...
  return (
    <div className={`bg-white dark:bg-slate-800 rounded-lg p-8 ${className}`}>
      {title && (
        <div className="mb-8">
            ...
            <div className="border-t border-gray-200 dark:border-slate-700 pt-8">
              <div className="flex items-center justify-between mb-6">
                <div>
                  ...
                  <p className="mt-1 text-sm text-gray-500 dark:text-gray-400">Add technical specifications for this product</p>
                </div>
                <button
                  type="button"
                  onClick={addSpec}
              ...
              <div className="space-y-4">
                {dynamicSpecs.map((spec, index) => (
                  <div key={spec.id} className="grid grid-cols-1 md:grid-cols-2 gap-4 p-4 border border-gray-200 dark:border-slate-700 rounded-lg bg-gray-50 dark:bg-slate-700/50">
                    <div>
                      <label className="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-2">
                        Spec Name
                      </label>
                      ...
                      {errors[`spec_${index}_name`] && (
                        <p className="mt-2 text-sm text-red-600 dark:text-red-400">{errors[`spec_${index}_name`]}</p>
                      )}
                    </div>
                    ...
                    <div className="relative">
                      ...
                      {errors[`spec_${index}_value`] && (
                        <p className="mt-2 text-sm text-red-600 dark:text-red-400">{errors[`spec_${index}_value`]}</p>
                      )}
                    </div>
                  </div>
                ))}
